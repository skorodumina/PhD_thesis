...
%\footnote[1]{See Eq.~\eqref{eq:err_subtr} in App.~\ref{app_uncert}.}
The absolute statistical  uncertainty due to the limited statistics of the experimental data is calculated in the non-empty bins as (see Eq.~\eqref{eq:err_subtr} in App.~\ref{app_uncert}).%\vspace{-0.25em}
\begin{equation}
...
\Delta W \! \cdot \!  \Delta Q^{2} \! \cdot \!  \Delta^{5} \tau \! \cdot \! \mathcal{L}},%\\[-8pt]
\label{staterrors}
\end{equation}
...
%\footnote[2]{See Eq.~\eqref{eq:err_frac} in App.~\ref{app_uncert}.}
...
% (see Eqs.~\eqref{inegr5diff} and~\eqref{eq:reported_sec})
...
\subsection{Uncertainty of the empty cells filling}
...
\looseness=-1
\subsection{Uncertainty~of~unfolding~the~effects~of~target~motion}
\label{Sect:mod_dep2}
